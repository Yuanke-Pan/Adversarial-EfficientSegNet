...
class SegDataSet(Dataset):
    ...
    def __init__(self, cfg, phase):
        ...
        if self.phase == 'val':
            ...
            self.db_file = self.cfg.DATA_LOADER.UNLABEL_DB_FILE
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'test':
        ...
        elif (self.phase == 'val' or not self.is_data_augment) and self.phase != 'semi':
            if self.is_coarse:
            ...
            if self.phase == 'train':
                ...
                # didn't modify augment function because it may heve no effect
                original_npy_image = npy_image
                if self.cfg.DATA_AUGMENT.IS_RANDOM_ROTATE:
                    npy_image = data_augmentor.random_rotate(npy_image, npy_label=None,
                                                            min_angle=self.cfg.DATA_AUGMENT.ROTATE_ANGLE[0],
                                                            max_angle=self.cfg.DATA_AUGMENT.ROTATE_ANGLE[1])
                if self.cfg.DATA_LOADER.IS_COARSE:
                    if self.cfg.DATA_AUGMENT.IS_RANDOM_SHIFT and np.random.choice([0, 1]):
                        npy_image = data_augmentor.random_shift(npy_image, None, self.cfg.DATA_AUGMENT.SHIFT_MAX_RATIO)
                    out_size = self.cfg.COARSE_MODEL.INPUT_SIZE
                else:
                    out_size = self.cfg.FINE_MODEL.INPUT_SIZE
                npy_image, _ = ScipyResample.resample_to_size(npy_image, out_size)
                original_npy_image, _ = ScipyResample.resample_to_size(original_npy_image, out_size)
                if self.cfg.DATA_AUGMENT.IS_ELASTIC_TRANSFORM and np.random.choice([0, 1]):
                    npy_image = data_augmentor.elastic_transform_3d(npy_image)

                if self.cfg.DATA_AUGMENT.IS_RANDOM_FLIP:
                    npy_image = data_augmentor.random_flip(npy_image)

                dis_original_npy_image = npy_image
                
                if self.cfg.DATA_AUGMENT.IS_CHANGE_ROI_HU:
                    npy_image = data_augmentor.augment_brightness_additive(npy_image, None,
                                                                           self.cfg.DATA_LOADER.LABEL_INDEX,
                                                                           self.cfg.DATA_AUGMENT.ROI_HU_RANGE)
                if self.cfg.DATA_LOADER.IS_NORMALIZATION_HU:
                    npy_image = clip_and_normalize_mean_std(npy_image, self.window_level[0], self.window_level[1])
                    dis_original_npy_image = clip_and_normalize_mean_std(dis_original_npy_image, self.window_level[0], self.window_level[1])
                    original_npy_image = clip_and_normalize_mean_std(original_npy_image, self.window_level[0], self.window_level[1])
                else:
                    npy_image = normalize_min_max_and_clip(npy_image, self.window_level[0], self.window_level[1])
                    dis_original_npy_image = normalize_min_max_and_clip(dis_original_npy_image, self.window_level[0], self.window_level[1])
                    original_npy_image = normalize_min_max_and_clip(original_npy_image, self.window_level[0], self.window_level[1])

                if self.cfg.DATA_AUGMENT.IS_ADD_GAUSSIAN_NOISE and np.random.choice([0, 1]):
                    data_augmentor = DataAugmentor()
                    npy_image = data_augmentor.augment_gaussian_noise(npy_image, noise_variance=(0, 0.1))

                image_czyx = npy_image[np.newaxis, ]
                if self.cfg.SEMI_SUP.MODE == 'Teacher':
                    original_npy_image = original_npy_image[np.newaxis,]
                    return torch.from_numpy(image_czyx).float(), torch.from_numpy(original_npy_image).float()
                else:
                    dis_original_npy_image = dis_original_npy_image[np.newaxis,]
                    return torch.from_numpy(image_czyx).float(), torch.from_numpy(dis_original_npy_image).float()
    ...
    def _read_db(self):
        ...
        for key, value in txn.cursor():
            ...
            tmp_data = MaskData(series_id=key,
                                ...
                                if 'mask_path' in label_info else None,
