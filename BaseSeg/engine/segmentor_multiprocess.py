...
class SegmentationMultiProcess(object):
    ...
    def run(self, process_idx, cfg):
        ...
        if self.is_distributed_train:
            ...
            if cfg.ENVIRONMENT.CUDA:
                ...
                self.semi_model = self.semi_model.to(self.device)
        else:
    ...
    def do_train(self, fold=1):
        ...
        #there can test semi_dataloader 
        _ = next(iter(self.semi_loader))
        ...
        if epoch < 50 or epoch > 450:       
            ...
            for index, (images, masks) in enumerate(self.train_loader):
                ...
                with torch.no_grad():
                    ...
                    aug_unlabel_image_score = dis_output_total[0][0]
                    unlabel_image_score = dis_output_total[0][1]
                ...
                if aug_unlabel_image_score < unlabel_image_score:
                    out_put = self.model(aug_unlabel_image)
                    loss_func = SegLoss(loss_func=self.cfg.TRAINING.LOSS, activation=self.cfg.TRAINING.ACTIVATION)
                    unlabel_seg_loss = loss_func(out_put, unlabel_image_mask) * unlabel_image_score * t
                else:
                    out_put = self.model(unlabel_image)
                    loss_func = SegLoss(loss_func=self.cfg.TRAINING.LOSS, activation=self.cfg.TRAINING.ACTIVATION)
                    unlabel_seg_loss = loss_func(out_put, aug_unlabel_mask) * aug_unlabel_image_score * t
