...
def run_prepare_data(cfg):
    ...
    for data in data_prepare.data_info:
        try:
            pool.apply_async(data_prepare.process, (data,'train',))
        except Exception as err:
            traceback.print_exc()
            print('Create coarse/fine image/mask throws exception %s, with series_id %s!' % (err, data.series_id))
    for data in data_prepare.unlabel_data_info:
        try:
            pool.apply_async(data_prepare.process, (data, 'unlabel',))
        except Exception as err:
            traceback.print_exc()
            print('Create coarse/fine unlabel image throws exception %s, with series_id %s!' % (err, data.series_id))
    pool.close()
    pool.join()
    print(data_prepare.Err_id)
...
# Unlabel modified
class unlabelData(object):
    def __init__(self, series_id, image_path, 
                coarse_image_path=None, fine_image_path=None):
        super(unlabelData, self).__init__()
...
class DataPrepare(object):
    def __init__(self, cfg):
        ...
        self.out_unlabel_db_file = self.db_dir + 'seg_unlabel_preprocess_database'
        ...
        # For Unlabel
        self.train_unlabel_db_file = self.db_dir + 'seg_raw_train_unlabel'
        self.unlabel_dir = cfg.DATA_PREPARE.TRAIN_UNLABEL_IMAGE_DIR

        if not os.path.exists(self.db_dir):
            os.makedirs(self.db_dir)
        self._create_db_file(phase='train')
        self._create_db_file(phase='test')
        self._create_db_file(phase='unlabel')
        self.data_info = self._read_db()
        self.unlabel_data_info = self._read_db(phase='unlabel')
    ...
    # Modified unlabel
    def process(self, data, phase):
        series_id = data.series_id
        image_info = load_ct_info(data.image_path)
        if phase == 'train':
            mask_info = load_ct_info(data.mask_path)
        print('Start processing %s.' % series_id, phase)
        if image_info == {} and phase != 'train':
            self.Err_id.append(series_id)
    ...
    def _read_db(self, phase='train'):
        if phase == 'train':
            ...
            local_data = []
            env = lmdb.open(self.train_unlabel_db_file, map_size=int(1e9))
            txn = env.begin()
            for key, value in txn.cursor():
                key = str(key, encoding='utf-8')
                value = str(value, encoding='utf-8')
                label_info = json.loads(value)
                tmp_data = unlabelData(key,
                                    label_info['image_path'])
                local_data.append(tmp_data)
            env.close()
    ...
    def _update_db(self, data, phase = 'train'):
        if phase == 'train':
            ...
            env = lmdb.open(self.out_unlabel_db_file, map_size=int(1e9))
        ...
        txn.commit()
        env.close()
    # Has modified for unlabel data
    def _create_db_file(self, phase='train'):
        # Modified for unlabel db create
        if phase == 'train':
            db_file_path = self.train_db_file
        elif phase == 'test':
            db_file_path = self.test_db_file
        else:
            db_file_path = self.train_unlabel_db_file
        DataBaseUtils.creat_db(db_file_path)
        # Modified for unlabel read txt
        if phase == 'train':
            series_ids = read_txt(self.cfg.DATA_PREPARE.TRAIN_SERIES_IDS_TXT)
        elif phase == 'test':
            series_ids = read_txt(self.cfg.DATA_PREPARE.TEST_SERIES_IDS_TXT)
        else:
            series_ids = read_txt(self.cfg.DATA_PREPARE.TRAIN_UNLABEL_SERIES_IDS_TXT)
        ...
        for series_id in series_ids:
            if phase == 'train':
                ...
                image_path = os.path.join(self.cfg.DATA_PREPARE.TRAIN_UNLABEL_IMAGE_DIR, series_id + '_0000.nii.gz')
                if os.path.exists(image_path):
                    data_dict = {'image_path': image_path}
                    DataBaseUtils.update_record_in_db(db_file_path, series_id, data_dict)
                else:
